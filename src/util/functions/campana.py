...
class Campana():
    ...
    def set_campana_variables(self, conn, nombre_campana):
        ...
        codigo_campana = conn.select(f"SELECT CODIGO_CAMPANA FROM {schema}.MON_CAMP_DESC WHERE NOMBRE = '{nombre_campana}'").iloc[0,0]
        ...
        vigencia_fin_mes_evo =pd.Timestamp(f"{vigencia_fin_campana.year}-{vigencia_fin_campana.month}-01") + pd.DateOffset(months=1) - pd.DateOffset(days=1)
    ...
    def get_campana_info(self, conn, nombre):
        codigo_campana = conn.select(f"SELECT CODIGO_CAMPANA FROM {schema}.MON_CAMP_DESC WHERE NOMBRE = '{nombre}'").iloc[0,0]  
        lis_df = []
        lis_queries = [
            f"SELECT * FROM {schema}.MON_CAMP_DESC WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"SELECT * FROM {schema}.MON_CAMP_LIST WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"SELECT * FROM {schema}.MON_CAMP_COUPON WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"SELECT * FROM {schema}.MON_CAMP_OFFER_CODE WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"SELECT DISTINCT CODIGO_CAMPANA, REGION, STATE, FORMATO_TIENDA, STORE_DESCRIPTION AS TIENDA, STORE_KEY, STORE_CODE FROM {schema}.MON_CAMP_STORE INNER JOIN {schema}.V_STORE USING(STORE_CODE) WHERE CODIGO_CAMPANA = '{codigo_campana}' ORDER BY 1,2,3,4"
            ,f"SELECT DISTINCT CODIGO_CAMPANA, PROVEEDOR, MARCA, CLASS_CODE, CLASS_DESC, SUBCLASS_CODE, SUBCLASS_DESC, PROD_TYPE_DESC, PRODUCT_DESCRIPTION, PRODUCT_CODE FROM {schema}.MON_CAMP_PRODUCT INNER JOIN {schema}.MON_CRM_SKU_RAZONSOCIAL USING(PRODUCT_CODE) LEFT JOIN DIM_PRODUCT USING(PRODUCT_CODE) WHERE CODIGO_CAMPANA = '{codigo_campana}' ORDER BY 1,2,3,4,5,6,7,8"
        ]
    ...
    def guardar_info_campana(self, conn, nombre_campana, table_name, df):
        ...
        try:
            ...
            conn.execute(f"DELETE {schema}.{table_name} WHERE CODIGO_CAMPANA = (SELECT DISTINCT CODIGO_CAMPANA FROM {schema}.MON_CAMP_DESC WHERE NOMBRE = '{nombre_campana}')")
            conn.insert(table_name, df)
    ...
    def eliminar_info_campana(self, conn, nombre_campana):
        codigo_campana = conn.select(f"SELECT CODIGO_CAMPANA FROM {schema}.MON_CAMP_DESC WHERE NOMBRE = '{nombre_campana}'").iloc[0,0]  
        lis_queries = [
            f"DELETE {schema}.MON_CAMP_DESC WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"DELETE {schema}.MON_CAMP_LIST WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"DELETE {schema}.MON_CAMP_COUPON WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"DELETE {schema}.MON_CAMP_OFFER_CODE WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"DELETE {schema}.MON_CAMP_STORE WHERE CODIGO_CAMPANA = '{codigo_campana}'"
            ,f"DELETE {schema}.MON_CAMP_PRODUCT WHERE CODIGO_CAMPANA = '{codigo_campana}'"
        ]
    ...
    def get_campaign_products(self, conn, nombre_campana):
        ...
        query = f"SELECT PRODUCT_CODE FROM {schema}.MON_CAMP_PRODUCT WHERE CODIGO_CAMPANA = (SELECT CODIGO_CAMPANA FROM {schema}.MON_CAMP_DESC WHERE NOMBRE = '{nombre_campana}')"
    ...
    def get_queries_campana_venta(self):
        ...
        query_venta_redencion = f'''
            ...
            WITH __FCT_SALE_LINE_DISCOUNT AS (
                ...
                INNER JOIN {schema}.MON_CAMP_OFFER_CODE B ON A.OFFER_CODE = B.OFFER_CODE AND CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}'
        ...
        query_indicadores_tx = f'''
            ...
            ,__DATOS_TX AS (
                ...
                INNER JOIN {schema}.MON_CAMP_STORE J ON A.STORE_CODE = J.STORE_CODE AND J.CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}'
                LEFT JOIN (SELECT DISTINCT INVOICE_NO, 1::INT AS IND_REGISTRO FROM {schema}.MON_CAMP_REGISTERS WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}') C USING(INVOICE_NO)
                ...
                LEFT JOIN (SELECT DISTINCT EMISSION_INVOICE_NO INVOICE_NO, 1::INT IND_CUP FROM FCT_OFFER_COUPON A INNER JOIN {schema}.MON_CAMP_COUPON B ON B.CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}' AND A.OFFER_CODE = B.COUPON_ID) USING(INVOICE_NO)
                WHERE (
                    INVOICE_DATE BETWEEN {self.campana_variables['date_dash']}
                OR INVOICE_DATE BETWEEN {self.campana_variables['date_dash_precampana']}
                OR INVOICE_DATE BETWEEN {self.campana_variables['date_dash_postcampana']}
        ...
        query_base_clientes = f'''
            ...
            FROM MAP_CUST_LIST
            INNER JOIN (SELECT DISTINCT LIST_ID, 'GT' GRUPO FROM {schema}.MON_CAMP_LIST WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}' UNION SELECT DISTINCT LIST_ID_GC, 'GC' GRUPO FROM {schema}.MON_CAMP_LIST WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}') USING(LIST_ID)
            )
            SELECT
                GRUPO
                ,LIST_ID
            ...
            INNER JOIN {schema}.MON_CAMP_COUPON C ON A.OFFER_CODE = C.COUPON_ID
            ...
            ,__BASE_ENVIOS AS(
                ...
                ,COUNT(OPEN_CUSTOMER_CODE) ABIERTOS --ABIERTOS --CLICKS
                FROM AGG_PROMO_CUST_RESPONSE A 
                INNER JOIN DIM_PROMOTION B USING(PROMOTION_CODE)
        ...
        query_venta = f'''
            DROP TABLE IF EXISTS {schema}.MON_VTA;
            CREATE TABLE {schema}.MON_VTA AS (
            SELECT
                1::INT AS IND_MC
                ,A.PRODUCT_CODE
                ,A.INVOICE_NO
            ...
            LEFT JOIN {schema}.V_CUSTOMER_CONTACT AS B ON A.CUSTOMER_CODE_TY = B.CUSTOMER_CODE-- AND B.CONTACT_INFO IS NOT NULL
            LEFT JOIN {schema}.V_STORE C ON A.STORE_KEY = C.STORE_KEY AND A.STORE_CODE = C.STORE_CODE
    ...
    def get_queries_campana_resultados(self):
        ...
        query_res_agg = f'''
                    ...
                    --GRUPO TRATADO
                    ,NULL::NUMERIC AS GT_CAMPANA_VENTA --SUM(CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN VENTA END) GT_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GT_CAMPANA_UNIDADES --SUM(CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN UNIDADES END) GT_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GT_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN INVOICE_NO END) GT_CAMPANA_TX
                    ,NULL::NUMERIC AS GT_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN CUSTOMER_CODE END) GT_CAMPANA_CLIENTES

                    --GRUPO CONTROL
                    ,NULL::NUMERIC AS GC_CAMPANA_VENTA --SUM(CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN VENTA END) GC_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GC_CAMPANA_UNIDADES --SUM(CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN UNIDADES END) GC_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GC_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN INVOICE_NO END) GC_CAMPANA_TX
                    ,NULL::NUMERIC AS GC_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN CUSTOMER_CODE END) GC_CAMPANA_CLIENTES
                    ...
                    --GRUPO TRATADO
                    --MES ANTERIOR
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_CAMPANA_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN VENTA END) GT_INCREMENTO_MES_ANTERIOR_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_CAMPANA_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN UNIDADES END) GT_INCREMENTO_MES_ANTERIOR_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN INVOICE_NO END) GT_INCREMENTO_MES_ANTERIOR_CAMPANA_TX
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN CUSTOMER_CODE END) GT_INCREMENTO_MES_ANTERIOR_CAMPANA_CLIENTES

                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GT' THEN VENTA END) GT_INCREMENTO_MES_ANTERIOR_VENTA
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GT' THEN UNIDADES END) GT_INCREMENTO_MES_ANTERIOR_UNIDADES
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GT' THEN INVOICE_NO END) GT_INCREMENTO_MES_ANTERIOR_TX
                    ,NULL::NUMERIC AS GT_INCREMENTO_MES_ANTERIOR_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GT' THEN CUSTOMER_CODE END) GT_INCREMENTO_MES_ANTERIOR_CLIENTES

                    --ANO_ANTERIOR
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN VENTA END) GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN UNIDADES END) GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN INVOICE_NO END) GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_TX
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GT' THEN CUSTOMER_CODE END) GT_INCREMENTO_ANO_ANTERIOR_CAMPANA_CLIENTES

                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GT' THEN VENTA END) GT_INCREMENTO_ANO_ANTERIOR_VENTA
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GT' THEN UNIDADES END) GT_INCREMENTO_ANO_ANTERIOR_UNIDADES
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GT' THEN INVOICE_NO END) GT_INCREMENTO_ANO_ANTERIOR_TX
                    ,NULL::NUMERIC AS GT_INCREMENTO_ANO_ANTERIOR_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GT' THEN CUSTOMER_CODE END) GT_INCREMENTO_ANO_ANTERIOR_CLIENTES

                    --GRUPO CONTROL
                    --MES ANTERIOR
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_CAMPANA_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN VENTA END) GC_INCREMENTO_MES_ANTERIOR_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_CAMPANA_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN UNIDADES END) GC_INCREMENTO_MES_ANTERIOR_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN INVOICE_NO END) GC_INCREMENTO_MES_ANTERIOR_CAMPANA_TX
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN CUSTOMER_CODE END) GC_INCREMENTO_MES_ANTERIOR_CAMPANA_CLIENTES

                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GC' THEN VENTA END) GC_INCREMENTO_MES_ANTERIOR_VENTA
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GC' THEN UNIDADES END) GC_INCREMENTO_MES_ANTERIOR_UNIDADES
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GC' THEN INVOICE_NO END) GC_INCREMENTO_MES_ANTERIOR_TX
                    ,NULL::NUMERIC AS GC_INCREMENTO_MES_ANTERIOR_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_MES_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'MES_ANTERIOR' AND GRUPO = 'GC' THEN CUSTOMER_CODE END) GC_INCREMENTO_MES_ANTERIOR_CLIENTES

                    --ANO_ANTERIOR
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN VENTA END) GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_VENTA
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN UNIDADES END) GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_UNIDADES
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN INVOICE_NO END) GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_TX
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'CAMPANA' AND GRUPO = 'GC' THEN CUSTOMER_CODE END) GC_INCREMENTO_ANO_ANTERIOR_CAMPANA_CLIENTES

                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_VENTA --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GC' THEN VENTA END) GC_INCREMENTO_ANO_ANTERIOR_VENTA
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_UNIDADES --SUM(CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GC' THEN UNIDADES END) GC_INCREMENTO_ANO_ANTERIOR_UNIDADES
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_TX --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GC' THEN INVOICE_NO END) GC_INCREMENTO_ANO_ANTERIOR_TX
                    ,NULL::NUMERIC AS GC_INCREMENTO_ANO_ANTERIOR_CLIENTES --COUNT(DISTINCT CASE WHEN IND_CLIENTE_CAMPANA = 1 AND IND_CLIENTE_ANO_ANTERIOR = 1 AND IND_MARCA = 1 AND A.PERIODO = 'ANO_ANTERIOR' AND GRUPO = 'GC' THEN CUSTOMER_CODE END) GC_INCREMENTO_ANO_ANTERIOR_CLIENTES
        ...
        query_res_insert = f'''
            --INSERTAR RESULTADOS
            DELETE {schema}.MON_CAMP_RESULTS WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_RESULTS SELECT * FROM #CAMP_RESULTS;

            --SELECT * FROM {schema}.MON_CAMP_RESULTS WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
        '''
    ...
    def get_queries_campana_datos_roi(self):
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_ROI WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_ROI SELECT * FROM #DATOS_ROI;
            '''
    ...
    def get_queries_campana_datos_funnel(self):
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_CONTACT_FUNNEL WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_CONTACT_FUNNEL SELECT * FROM #FUNNEL_SEMANAL ORDER BY 1,2,3,4;
            '''
        
        return [query_base_clientes, query_venta_cliente_semanal, query_funnel_semanal, query_insert]
    ...
    def get_queries_campana_tendencia(self):
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_TENDENCY WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_TENDENCY SELECT * FROM #TENDENCIA;
        '''
    ...
    def get_queries_campana_evolucion(self):        
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_EVOLUTION_SALES WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_EVOLUTION_SALES SELECT * FROM #VENTA_MENSUAL;
        '''
    ...
    def get_queries_campana_segmentos(self):
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_EVOLUTION_SEGMENTS WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_EVOLUTION_SEGMENTS SELECT * FROM #SEGMENTOS;
        '''
    ...
    def get_queries_campana_retencion(self):
        ...
        query_insert = f'''
            DELETE {schema}.MON_CAMP_EVOLUTION_RETENTION WHERE CODIGO_CAMPANA = '{self.campana_variables['codigo_campana']}';
            INSERT INTO {schema}.MON_CAMP_EVOLUTION_RETENTION SELECT * FROM #SEGMENTOS_RETENCION;
        '''
